...
#include "declarations_A.h"

using namespace std;

// constructor: intialise file path
Parser::Parser(string file_path) { path_to_file = file_path; }

bool Parser::is_a_ctype(std::string lookup) {
...
void Parser::increment_count_if_found(
    string name,
    string scope) {  // if variable or function found, increments each time that
                     // specific identifier has been found. Takes scope into
                     // account.
  for (int i = 0; i < identifiers.size(); i++) {
    Identifier var = identifiers[i];

    if (var.name == name && var.scope == scope) {
      identifiers[i].number_of_references =
          identifiers[i].number_of_references + 1;
      return;
    } else if (var.name == name && var.is_a_function) {
      identifiers[i].number_of_references =
          identifiers[i].number_of_references + 1;
      return;
    }
...
void Parser::parse_inline_declarations(
    vector<string> line, Function function, int line_number,
    int line_index) {  // needs to find in line declarations by detecting open
                       // brackets.
  ...
  for (size_t token_index = 0; token_index < line.size() - 1; token_index++) {
    ...
    if (token == "(") {  // found declaration in line
      opening_bracket_found = true;
    } else if (token == ")") {  // end of declaration
      ...
      identifiers.push_back(
          current_variable);  // stores into current_variable struct which is
                              // linked to the original identifier struct
    }
  }
}

void Parser::check_line_for_declarations(
    int line_index, vector<string> line, int line_number,
    Function function) {  // checking to see if it is an in line variable
  ...
  if (is_function) {
    ...
    identifiers.push_back(current_variable);
    ...
    if (line.back() ==
        ";") {  // to detect long long by parsing the line and checking the 1st
                // and 2nd character for long and long, if last character in the
                // line is a semi-colon
...
void Parser::parse() {  // opens new .txt file called identifiers and trunicates
  ...
  for (int function_index = 0; function_index < function_list.size();
    ...
    if (function_index > 0) {
      ...
      // and i have ignored new line characters this is needed to correct the
      // count
      line_number++;
    }
    ...
    for (int line_index = 0; line_index < function.size(); line_index++) {
      ...
      // goes through each line to check if a function declaration
      bool is_function_declaration =
          line_index == 0 && is_a_ctype(line[0]) && line.back() != ";";
      if (!is_function_declaration) {  // if it isn't a function declaration
                                       // check each token and previous token
        for (int token_idx = 0; token_idx < line.size(); token_idx++) {
          string token = line[token_idx];
          string prev_token = "";
          if (token_idx > 0) prev_token = line[token_idx - 1];
          // checks against regular expression to see if it isn't a c type and
          // has a-z,A-Z, _, ] and + if it is then it is the function name.
          if (!is_a_ctype(token) && !is_a_ctype(prev_token) &&
              regex_match(token, regex("[a-zA-Z_]+"))) {
            string function_name = function[0][1];
            increment_count_if_found(
                token,
                function_name);  // store the token in function name string
          }
        }
      }
    }
